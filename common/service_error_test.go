...
func TestNewServiceError(t *testing.T) {
	err := NewServiceError("TEST_CODE", "Something went wrong")
	...
	expectedErrorString := "[TEST_CODE] Something went wrong"
...
func TestIsCode_Match(t *testing.T) {
	err := fmt.Errorf("wrapped: %w", NewServiceError("MATCH_CODE", "match test"))
...
func TestIsCode_NoMatch(t *testing.T) {
	err := fmt.Errorf("wrapped: %w", NewServiceError("DIFFERENT_CODE", "no match"))
